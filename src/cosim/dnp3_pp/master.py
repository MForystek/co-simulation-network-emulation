...
from cosim.dnp3_pp.station_utils import parsing_gvid_to_gvcls, parsing_gv_to_mastercmdtype
from cosim.dnp3_pp.station_utils import collection_callback, command_callback, restart_callback

FILTERS = opendnp3.levels.NORMAL | opendnp3.levels.ALL_COMMS

_log = getLogger(__name__, "logs/d_pp_master.log")
...
class MyMasterNew:
    ...
    def __init__(self,
        ...
        self.soe_handler: SOEHandler = SOEHandler(station_ref=self) if soe_handler is None else soe_handler
...
class SOEHandler(opendnp3.ISOEHandler):
    ...
    def Process(self, info,
        ...
        if str(info.gv) == "GroupVariation.Group30Var1":
            voltage_0 = visitor.index_and_value[0][1]/1000 # p.u.
            voltage_1 = visitor.index_and_value[1][1]/1000 # p.u.
...
def main():
    app = MyMasterNew(outstation_ip="172.17.0.1", port=20002)
