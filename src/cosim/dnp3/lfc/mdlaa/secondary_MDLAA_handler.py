...
from cosim.dnp3.lfc.mdlaa.constants import NUM_ATTACKED_LOAD_BUSES, NUM_OF_LOADS_SECONDARY_HANDLER
...
class MDLAAHandlerSecondary(SOEHandlerAdjusted):
    def __init__(self, log_file_path="logs/soehandler.log", soehandler_log_level=logging.INFO, station_ref=None, attack=np.zeros(NUM_ATTACKED_LOAD_BUSES), *args, **kwargs):
        ...
        # Constants
        self._NUM_OF_ATTACKED_LOADS = NUM_OF_LOADS_SECONDARY_HANDLER
    ...
    def _do_attack(self):
        loads = self._curr_attack[NUM_ATTACKED_LOAD_BUSES - self._NUM_OF_ATTACKED_LOADS:]
