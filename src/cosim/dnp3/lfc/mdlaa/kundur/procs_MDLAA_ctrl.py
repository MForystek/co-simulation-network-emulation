...
class MDLAAHandler:
    def __init__(self, main_to_master1: Queue, main_to_master2: Queue, main_to_osqp:Queue, osqp_to_main:Queue):
        ...
        # Constants
        self._NUM_ATTACKED_LOADS_MASTER1 = NUM_LOADS_MASTER1_KUNDUR
        ...
        self._SINUS_ANGLES = np.random.uniform(0, 2*np.pi, NUM_ATTACKED_LOADS_KUNDUR) # pu of load
        ...
        self._curr_freqs = np.ones(NUM_GENS_KUNDUR)
        self._curr_attack = np.ones(NUM_ATTACKED_LOADS_KUNDUR, dtype=np.float32) # Used also to apply the attack through the second master station
    
        # History of max and min attacks
        self._all_max_attack = np.ones(NUM_ATTACKED_LOADS_KUNDUR)
        self._all_min_attack = np.ones(NUM_ATTACKED_LOADS_KUNDUR)
    
        # Data storage for historical attacks and frequencies
        self._U = np.empty([NUM_ATTACKED_LOADS_KUNDUR, Ta_KUNDUR])
        self._Y = np.empty([NUM_GENS_KUNDUR, Ta_KUNDUR])
        self._attack_history = np.ones([NUM_ATTACKED_LOADS_KUNDUR, Tini]) # Stores Tini past attacks
        self._freq_history = np.ones([NUM_GENS_KUNDUR, Tini])             # Stores Tini past frequencies 
    ...
    def _read_frequencies(self, incoming_data):
        freqs = [incoming_data[key][1] for key in range(NUM_GENS_KUNDUR)]
        for i in range(NUM_GENS_KUNDUR):
    ...
    def _generate_and_apply_random_attack(self):
        ...
        rnd_attack = np.random.uniform(-self._RND_ATTACK, self._RND_ATTACK, NUM_ATTACKED_LOADS_KUNDUR)
    ...
    def _correct_attacks_beyond_bounds(self):
        for i in range(NUM_ATTACKED_LOADS_KUNDUR):
...
def main():    
    ...
    master2 = Process(target=master2_process, args=(main_to_master2, step_time,
                                                    NUM_ATTACKED_LOADS_KUNDUR, NUM_LOADS_MASTER2_KUNDUR))
    master2.start()
    
    osqp = Process(target=osqp_process, args=(master_to_osqp, osqp_to_master,
                                              NUM_GENS_KUNDUR, NUM_ATTACKED_LOADS_KUNDUR,
