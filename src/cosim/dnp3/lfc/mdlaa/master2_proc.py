...
class MDLAAHandlerSecondary(SOEHandlerAdjusted):
    def __init__(self, station_ref, attack, num_attacked_loads, num_loads_secondary_handler, log_file_path="logs/d_r_lfc_mdlaa.log", soehandler_log_level=logging.INFO, *args, **kwargs):
        super().__init__(log_file_path, soehandler_log_level, station_ref, *args, **kwargs)
        
        # Constants
        self._num_attacked_load_buses = num_attacked_loads
        self._num_of_loads_secondary_handler = num_loads_secondary_handler
        
        # MDLAA attack storage
        self._curr_attack = attack
        self._prev_attack = np.ones(num_attacked_loads)
...
def master2_process(main_to_master2: Queue, step_time, num_attacked_loads, num_loads_master2):
    ...
    soe_handler2 = MDLAAHandlerSecondary(station_ref=master2, attack=loads_coeffs,
                                         num_attacked_loads=num_attacked_loads,
                                         num_loads_secondary_handler=num_loads_master2)
